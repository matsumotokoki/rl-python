...
class Q_learnning(object):
    def __init__(self,env_str="MountainCar-v0",f_probability=0.5, max_epi=100000, step_by_epi=200, goal_ave=-150, digitized_num=30):
        ...
        self.q_table = np.random.uniform(low=-1,high=1,size=(digitized_num**self.ndim_obs,self.env.action_space.n))
        self.gamma = 0.99
        self.alhpa = 0.55
    ...
    def run(self):
        for episode in range(self.max_episodes):
            ...
            for i in range(self.step_by_episode):
                if self.learining_is_done or i > self.max_episodes - 30:
                    self.env.render()
                
                self.obs,reward,done,info = self.env.step(action)
                # if i < 195 and done:  #CartPole 
                #     reward = -200
